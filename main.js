...
// Call the function to display Player One and Player Two buttons
// and create boxes only when index2Content is displayed
function displayPlayerButtonsAndBoxes() {
  const index1Content = document.getElementById("index1Content");
  const index2Content = document.getElementById("index2Content");

  // We need to ensure that index2Content is part of the HTML and styled with display: none initially.
  // We should not run the below functions unless index2Content is meant to be shown.
  if (
    index1Content &&
    index2Content &&
...
function showIndex2() {
  // Hide index1.html elements
  const index1Content = document.getElementById("index1Content");
  if (index1Content) {
    index1Content.style.display = "none";
  }

  // Show index2.html elements
  const index2Content = document.getElementById("index2Content");
  if (index2Content) {
    index2Content.style.display = "block"; // Or "grid" or whatever is appropriate
    displayPlayerButtonsAndBoxes(); // Call here after index2Content is visible
  }
}
createBoxBelowPlayerOne();
createBoxBelowPlayerTwo();
